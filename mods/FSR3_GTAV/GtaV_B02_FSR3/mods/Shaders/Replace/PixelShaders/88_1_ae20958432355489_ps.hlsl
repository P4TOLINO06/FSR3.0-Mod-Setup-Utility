...
{
  float reflectivePower : packoffset(c0);
...
SamplerState EnvironmentSampler_s : register(s2);
Texture2D<float4> DiffuseSampler : register(t0);
Texture2D<float4> EnvironmentSampler : register(t2);
  ...
  out float2 motionVectors : SV_Target4)
{
  ...
  r0.y = rsqrt(r0.y);
  r0.xy = r0.xz * r0.yy + float2(1,1);
  r0.xy = float2(-0.5,-0.5) * r0.xy;
  r1.xyzw = EnvironmentSampler.Sample(EnvironmentSampler_s, r0.xy).xyzw;
  r0.xyz = reflectivePower * r1.xyz;
  ...
  r0.w = r1.y * r0.w;
  r1.y = r0.w * -0.5 + 1;
  r2.xy = float2(0.5,0.48828125) * r0.ww;
  o2.xy = sqrt(r2.xy);
