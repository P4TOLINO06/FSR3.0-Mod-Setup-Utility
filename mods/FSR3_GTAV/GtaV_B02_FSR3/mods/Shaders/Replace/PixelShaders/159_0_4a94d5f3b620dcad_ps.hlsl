...
SamplerState GBufferTextureSamplerSSAODepth_s : register(s4);
SamplerState HDRSampler_s : register(s5);
Texture2D<float4> GBufferTextureSamplerSSAODepth : register(t4);
  ...
  out float2 motionVectors : SV_Target4)
{
  ...
  r0.xyzw = GBufferTextureSamplerSSAODepth.Sample(GBufferTextureSamplerSSAODepth_s, v1.xy).xyzw;
  r0.x = dot(v0.xy, float2(0.5,0.5));
  r0.x = frac(r0.x);
  r0.x = cmp(r0.x < 0.5);
  r0.yz = TexelSize.xy * float2(1,0) + v1.xy;
  r1.xyzw = GBufferTextureSamplerSSAODepth.Sample(GBufferTextureSamplerSSAODepth_s, r0.yz).wxyz;
  if (r0.x == 0) {
    r1.yz = cmp(r0.ww == float2(0,1));
    r1.y = (int)r1.z | (int)r1.y;
    if (r1.y != 0) {
      r2.xyzw = HDRSampler.Sample(HDRSampler_s, v1.xy).xyzw;
    }
    r1.x = 1;
  } else {
  ...
  r3.xyzw = HDRSampler.Sample(HDRSampler_s, r0.yz).xyzw;
  if (r1.y == 0) {
    r4.xyzw = HDRSampler.Sample(HDRSampler_s, v1.xy).xyzw;
    r0.y = cmp(r0.w == 0.000000);
    r0.y = r0.y ? 1 : r0.w;
    r1.yzw = r4.xyz + -r3.xyz;
    r0.yzw = r0.yyy * r1.yzw + r3.xyz;
    r1.yzw = -r4.xyz + r3.xyz;
