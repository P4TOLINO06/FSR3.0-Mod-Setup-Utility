...
SamplerState GBufferTextureSampler0_s : register(s2);
SamplerState GBufferTextureSamplerDepth_s : register(s3);
Texture2D<float4> GBufferTextureSampler0 : register(t2);
  ...
  out float2 motionVectors : SV_Target4)
{
  ...
  r0.xy = globalScreenSize.xy * v1.xy;
  r0.xy = (int2)r0.xy;
  r0.zw = float2(0,0);
  r0.xy = StencilCopyTexture.Load(r0.xyz).xy;
  r0.x = (uint)r0.y;
  r0.yz = cmp(r0.xx >= float2(3,23));
  r0.xw = cmp(r0.xx < float2(4,24));
  r0.xy = r0.xw ? r0.yz : 0;
  r0.x = (int)r0.y | (int)r0.x;
  r1.xyzw = GBufferTextureSampler0.Sample(GBufferTextureSampler0_s, v1.xy).xyzw;
  r0.yzw = r1.xyz * float3(2,2,2) + float3(-1,-1,-1);
  r0.y = dot(r0.yzw, r0.yzw);
  r0.y = rsqrt(r0.y);
  r0.y = r0.w * r0.y;
  r0.y = cmp(r0.y < 0.970000029);
  r0.x = (int)r0.y | (int)r0.x;
  r1.xyzw = GBufferTextureSamplerDepth.Sample(GBufferTextureSamplerDepth_s, v1.xy).xyzw;
  r0.y = 1 + dofProj.w;
  r0.y = r0.y + -r1.x;
  r0.y = dofProj.z / r0.y;
  r0.z = HeatHazeParams.x + HeatHazeParams.y;
  r0.z = cmp(r0.y < r0.z);
  r0.y = cmp(HeatHazeParams.x < r0.y);
  r0.y = r0.z ? r0.y : 0;
  r0.y = r0.y ? 1.000000 : 0;
  o0.xyzw = r0.xxxx ? float4(0,0,0,0) : r0.yyyy;
  return;
}