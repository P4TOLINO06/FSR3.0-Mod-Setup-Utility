  ...
  out float2 motionVectors : SV_Target4)
{
  ...
  r0.w = v3.w * r0.w;
  r1.x = cmp(StubbleGrowth.y != 0.000000);
  if (r1.x != 0) {
    r1.x = cmp(_matMaterialColorScale[1].x == 1.000000);
    if (r1.x != 0) {
      r1.xy = detailSettings.zz * v1.xy;
      r1.z = 0;
      r1.xyzw = VolumeSampler.Sample(VolumeSampler_s, r1.xyz).xyzw;
    } else {
      r2.xy = detailSettings.zz * v1.xy;
      r2.z = 0;
      r2.xyzw = VolumeSampler.Sample(VolumeSampler_s, r2.xyz).xyzw;
      r3.xy = float2(2,2) * v1.xy;
      r3.z = 0;
      r3.xyzw = VolumeSampler.Sample(VolumeSampler_s, r3.xyz).xyzw;
      r1.y = -r3.x + r2.x;
      r1.x = _matMaterialColorScale[1].x * r1.y + r3.x;
    }
    r1.y = -2 + detailSettings.x;
    r1.y = _matMaterialColorScale[1].x * r1.y + 2;
    r1.z = -2 + StubbleGrowth.y;
  ...
  r1.x = frac(r1.x);
  r1.x = cmp(r1.x < 0.5);
  r1.yz = cmp(float2(0.949999988,0.501960814) >= r0.ww);
  r1.x = r1.y ? r1.x : 0;
