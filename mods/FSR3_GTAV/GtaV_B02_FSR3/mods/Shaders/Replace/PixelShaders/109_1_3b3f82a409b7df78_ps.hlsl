  ...
  out float2 motionVectors : SV_Target4)
{
  ...
  r0.x = cmp(r0.x < 1);
  if (r0.x != 0) discard;
  r0.x = cmp(gAlphaRefVec0.x >= globalScalars.x);
  if (r0.x != 0) discard;
  ...
  r2.xyz = r1.www * v3.xyz + r2.xyz;
  r1.w = dot(r2.xyz, r2.xyz);
  r1.w = rsqrt(r1.w);
  r2.xyw = r2.xyz * r1.www;
  r3.xyzw = r1.xyxy * r1.xyxy;
  r3.xyz = float3(specularIntensityMult, specularFalloffMult, specularFresnel) * r3.xyz;
  r4.x = r3.x * r1.z;
  o1.w = saturate(v3.z * 128 + -127);
  r1.x = 255.009995 * r0.w;
  r1.x = floor(r1.x);
  r1.y = -32 + r1.x;
  r5.x = 0.0078125 * r1.y;
  r5.y = paletteSelector;
  r5.xyzw = TextureSamplerDiffPal.Sample(TextureSamplerDiffPal_s, r5.xy).xyzw;
  r1.y = cmp(r1.x >= 32);
  r1.x = cmp(160 >= r1.x);
  r1.x = r1.x ? r1.y : 0;
  ...
  r1.x = specular2ColorIntensity * r3.z;
  r1.y = dot(v4.xyz, v4.xyz);
  r1.y = rsqrt(r1.y);
  r6.xyz = specular2Color.xyz * r1.xxx;
  r7.xyz = v4.xyz * r1.yyy + -gDirectionalLight.xyz;
  r1.x = dot(r7.xyz, r7.xyz);
  r1.x = rsqrt(r1.x);
  r7.xyz = r7.xyz * r1.xxx;
  r1.x = dot(r2.xyw, r7.xyz);
  r1.x = saturate(9.99999994e-09 + r1.x);
  r1.y = r3.w * specular2Factor + 9.99999994e-09;
  r1.x = log2(r1.x);
  r1.x = r1.y * r1.x;
  r1.x = exp2(r1.x);
  r0.xyz = r6.xyz * r1.xxx + r0.xyz;
  o0.w = globalScalars.x * r0.w;
  o1.xyz = r2.xyw * float3(0.5,0.5,0.5) + float3(0.5,0.5,0.5);
  r4.y = 0.001953125 * r3.y;
  r5.x = v1.x * globalScalars.z + gLightArtificialIntAmbient0.w;
  r1.xy = float2(0.5,0.5) * r5.xz;
  o3.xy = sqrt(r1.xy);
  r0.w = r2.z * r1.w + -0.349999994;
  ...
  r0.x = saturate(r3.x * r1.z + 0.400000006);
